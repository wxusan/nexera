...
const service = SERVICE_DETAILS['university-admissions'];

export default function UniversityAdmissionsPage() {
  const [isAccordionOpen, setIsAccordionOpen] = ([] as any[]).length ? [false, false, false, false] : Array(service.documents.length).fill(false);
  ...
  return (
    <>
                ...
                {service.whatsIncluded.map((item, index) => (
                  ...
                  >
                    <div className="w-6 h-6 bg-primary-100 rounded-full flex items-center justify-center flex-shrink-0 mt-1">
                      <Icons.Check size={16} color="#2563EB" />
                    </div>
            ...
            <FadeInSection direction="right">
              <div className="bg-gradient-to-br from-primary-50 to-blue-50 p-8 rounded-xl">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-white p-4 rounded-lg">
                    <p className="text-sm text-gray-600 mb-1">Universities Partnered</p>
                    <p className="text-2xl font-bold text-primary-600">80+</p>
                  </div>
                  <div className="bg-white p-4 rounded-lg">
          ...
          <div className="space-y-6">
            {service.steps.map((step, index) => (
              <FadeInSection key={step.step} delay={index * 0.1}>
                ...
                >
                  <div className="w-12 h-12 bg-primary-100 rounded-lg flex items-center justify-center flex-shrink-0 font-bold text-primary-600">
                    {step.step}
                  </div>
                  <div>
          ...
          <div className="space-y-4">
            {service.documents.map((doc, index) => (
              <FadeInSection key={doc} delay={index * 0.05}>
                <motion.div
                  className="bg-light p-4 rounded-lg border border-gray-200 hover:border-primary-300 transition-colors"
                  whileHover={{ y: -2 }}
                >
                  <div className="flex items-center gap-3">
                    <Icons.FileText size={20} className="text-primary-600 flex-shrink-0" />
          ...
          <FadeInSection>
            <div className="text-center mb-12">
              ...
              <p className="section-subheading">
                Hear from students who achieved their admission goals with us.
              </p>
            </div>
          </FadeInSection>
      ...
      {/* CTA Banner */}
      <CTABanner
        title="Ready to Start Your University Application?"
        subtitle="Our expert consultants will guide you through every step of the process."
        buttonText="Start Application Now"
        buttonHref="/apply?service=university-admissions"
      />
    </>
  );
}
