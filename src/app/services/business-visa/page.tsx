...
const service = SERVICE_DETAILS['business-visa'];

export default function BusinessVisaPage() {
  return (
    <>
      {/* Hero Section */}
      <section className="section-padding bg-gradient-to-br from-purple-50 to-indigo-50 pt-32 sm:pt-40">
                ...
                {service.whatsIncluded.map((item, index) => (
                  ...
                  >
                    <div className="w-6 h-6 bg-purple-100 rounded-full flex items-center justify-center flex-shrink-0 mt-1">
                      <Icons.Check size={16} color="#8B5CF6" />
                    </div>
              ...
              <div className="bg-gradient-to-br from-purple-50 to-indigo-50 p-8 rounded-xl">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-white p-4 rounded-lg">
                    ...
                    <p className="text-2xl font-bold text-purple-600">3-5 Weeks</p>
                  </div>
                  <div className="bg-white p-4 rounded-lg">
          ...
          <div className="space-y-6">
            {service.steps.map((step, index) => (
              <FadeInSection key={step.step} delay={index * 0.1}>
                ...
                >
                  <div className="w-12 h-12 bg-purple-100 rounded-lg flex items-center justify-center flex-shrink-0 font-bold text-purple-600">
                    {step.step}
                  </div>
                  <div>
          ...
          <div className="space-y-4">
            {service.documents.map((doc, index) => (
              <FadeInSection key={doc} delay={index * 0.05}>
                <motion.div
                  className="bg-light p-4 rounded-lg border border-gray-200 hover:border-purple-300 transition-colors"
                  whileHover={{ y: -2 }}
                >
                  <div className="flex items-center gap-3">
                    <Icons.FileText size={20} className="text-purple-600 flex-shrink-0" />
          ...
          <div className="space-y-4">
            {service.faq.map((item, index) => (
              <FadeInSection key={index} delay={index * 0.05}>
                <motion.div
                  className="bg-white rounded-xl overflow-hidden border border-gray-200 hover:border-purple-300 transition-colors"
                  whileHover={{ y: -2 }}
                >
                  <div className="p-6 cursor-pointer hover:bg-purple-50 transition-colors">
                    <h3 className="font-bold text-navy flex items-center gap-2">
                      <Icons.HelpCircle size={18} className="text-purple-600 flex-shrink-0" />
                      {item.q}
                    </h3>
      ...
      {/* CTA Banner */}
      <CTABanner
        title="Facilitate Your Business Travel"
        subtitle="Get your business visa processed efficiently by our expert team."
        buttonText="Apply for Business Visa"
        buttonHref="/apply?service=business-visa"
      />
    </>
  );
}
